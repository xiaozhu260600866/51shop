<template>
	<view>
		...
		<view class="pb60" v-if="data.show">
			<view class="className" @click="goto('/pages/user/demand/class/main')" v-if="!ruleform.id">{{childrenName}}</view>
			<view class="className" @click="goto('/pages/user/demand/class/main?id='+ruleform.id)" v-if="ruleform.id">{{childrenName}}</view>
			...
			<view class="mb12 bg-f text">
				...
				<weui-input v-model="ruleform.address" label="地址" type="location" name="address" datatype="require" @callback="locationRes"></weui-input>
				...
				<weui-input v-model="ruleform.amount" myclass="xiaozhu2" label="发贴费用" type="txt" name="amount" :disabled="true" ></weui-input>
			</view>
			...
			<view class="pay_method bg-f mb12 fs-16" v-if="!ruleform.id">
				<weui-input v-model="ruleform.pay_method" name="pay_method" datatype="require" label="支付方式" changeField="value" type="radio"
			...
			<dxftButton size="lg" type="success" @click="submit">确认发布</dxftButton>
			...
			ajax() {
				this.getAjax(this).then(msg => {
					...
					if (msg.detail && !this.ruleform.title) {
						...
						if(this.data.user.role == 5){
							this.ruleform.name = msg.user.userInfo.company_name;
							this.ruleform.phone = msg.user.userInfo.phone;
						}else{
							this.ruleform.name = msg.user.nickname;
						}
