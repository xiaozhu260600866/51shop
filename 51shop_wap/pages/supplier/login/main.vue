			...
			<div v-if="data.show">
				...
				<div class="login-list m15" v-if="data.show">
					...
					<div class="login02" v-if="showType == 1">
						<weui-input v-model="ruleform.username" label="帐号" placeholder="请输入帐号" type="text" name="username" datatype="require"></weui-input>
						<weui-input v-model="ruleform.password" label="密码" type="password" name="password" datatype="require"></weui-input>
						<div class="login-btn mt20">
							<myform :ruleform="ruleform" :vaildate="vaildate" @callBack="formSubmit" myclass="dx-btn dx-btn-big dx-btn-green bdr4 w-b100 fs-17"
							 title="登录"></myform>
							<p class="forget fs-15 fc-9 text-right pt15">忘记密码</p>
						</div>
					</div>
				</div>
			</div>
	...
	export default {
		...
		methods: {
			...
			formSubmit() {
				...
				this.postAjax("/user/supplier-for-login", this.ruleform).then(msg => {
					if (msg.data.status == 2) {
						...
						this.goto("/pages/supplier/index/main", 1);
						return false;
					}
				});
			}
