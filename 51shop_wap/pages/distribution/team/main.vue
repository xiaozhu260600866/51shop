...
import dxTabsDate from "doxinui/components/tabs/tabs_date"
	export default {
		...
		onPullDownRefresh() {
			...
			this.postAjax("/shop/dis/dis-count",{searchType:this.searchType}).then(msg=>{
				this.countShow = true;
		...
		methods: {
			chageDate(e){
				 if(e.start_date){
					 this.postAjax("/shop/dis/dis-count",{created_at_start:e.start_date,created_at_end:e.end_date}).then(msg=>{
					 	this.countShow = true;
					 	this.countData = msg.data;
					 });
				 }else{
					 this.postAjax("/shop/dis/dis-count",{searchType:e.prop}).then(msg=>{
					 	this.countShow = true;
					 	this.countData = msg.data;
					 });
				 }
