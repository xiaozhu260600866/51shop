		...
		<view class="content" v-if="data.show">
			<view class="bg-f">
				<view class="row_border">
					<dx-list-msg :imgSrc="detail.getLogo" :name="detail.name" noborder :imgR="6">
			...
			<view class="mt12 info bg-f">
				<dx-list-cell name="地址：" nameColor="#666" iconName="location-fill" :iconSize="18" iconColor="#33c45d" v-if="detail.address">
					<view slot="right" class="text-left flex-between flex-middle">
						<view class="Arial fs-15 fc-6 pr10 lh-20">{{detail.address}}</view>
						<dx-button type="success" size="small" block @click="location(detail.location_x,detail.location_y,detail.address)">导航</dx-button>
					</view>
				</dx-list-cell>
				<dx-list-cell name="电话：" nameColor="#666" iconName="tel-fill2" :iconSize="18" iconColor="#33c45d">
					<view slot="right" class="text-left flex-between flex-middle">
						<view class="Arial fs-15 fc-6 pr10">{{detail.phone}}</view>
						<dx-button type="success" size="small" block @click="phone(detail.phone)">拨号</dx-button>
					</view>
				</dx-list-cell>
			</view>
			...
			<view class="hongbao-list mtb12 bg-f" v-if="data.detail.red_packet">
				...
				<view>
					<dx-list-cell :name="item.getUser.nickname" :imgSrc="item.getUser.headimgurl" :imgWidth="26" :imgHeight="26" :imgR="6" v-for="item in data.lists.data">
						...
						<text slot="con" >
							<text class="pl5 fc-red fw-bold" v-if="item.id == maxId">手气最佳</text>
						</text>
					</dx-list-cell>
				</view>
			</view>
			<view id="show_footer">
				<view class="left plr8 bd-te">
					...
					<button class="btn-item cart" hover-class="none" @click="collection(detail)">
						<p :class="['iconfont',collect == true? 'icon-staryes':'icon-starno']" ></p>
						<p class="txt">{{collect == true?'已收藏':'收藏'}}</p>
					</button>
				</view>
				<view class="right flex1 w-b100 pr5">
					<view class="r-nav">
						<view class="r-item r-item-green fs-14" @click="phone(detail.phone)">一键拨号</view>
					</view>
					<view class="r-nav">
						<view class="r-item r-item-blue fs-14" @click="goto('/pages/business/show/main?id='+detail.getUser.id,1)">购买</view>
					</view>
				</view>
			</view>
			...
			<shareShow :data="data" ref="shareShow" :qrcodeFilePath="data.qrcode" :posterFilePath="getSiteName + '/upload/images/articleOrder/800_'+data.detail.poster_cover"></shareShow>
...
<script>
	...
	import shareShow from "@/components/poster/shareShow.vue";
	export default {
		components: {
			uParse,dxListCell,dxImages,dxListMsg,dxDiag,shareShow
		},
		data() {
			return {
		...
		methods: {
			...
			ajax() {
				this.getAjax(this, {
					...
					//如果领完了，就计算最大
					if(this.data.detail.getRedPacket.red_packet_num <=  this.data.packetOrdersCount){
						this.maxId = 0;
						let maxAmount = 0;
		...
		onShareAppMessage() {
			...
			return this.shareSource(this, this.data.detail.red_packet ? '领红包，享受免费套餐！':'456在线',1);
		},
